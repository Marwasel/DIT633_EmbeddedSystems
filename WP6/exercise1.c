...
void setup() {
  ...
  // attach an interrupt to the ENCA pin to read the encoder signals.
  // RISING goes low to high
...
int getAction(int error){
   ...
   // u_out is the output of the controller.
  // kp is the proportional constant used in the controller to adjust the speed 
  // that is based on the error between the desired position and the actual position. 
 u_out = kp * error;  
...
void ISR_readEncoder(){
  // read encounter signals from ENCA
  a = digitalRead(ENCA);  
  // Read the encoder signals from ENCB
  b = digitalRead(ENCB);  
  // motor rotates to the right when a is greater than b, 
  if (a > b) 
   // position is increased
    pos++;    
  // motor rotates to the left, when b is greater than a
  else    
    // position is decreseased 
    pos--;
}