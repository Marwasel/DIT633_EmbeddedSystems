...
#endif
// end of the line for the time library 
int tempPin = A0;
long voltage; // declaring the volatge 
int celsuis = 0; // declaring the temperature
volatile int flag = 0; // Variable for flag 
const int led_3 = 6; //declaring the led to be pin 3
const int led_4 = 2; // declaring the led to be pin 4
const int led_5 = 3; // declaring the led to be pin 5
const int led_6 = 4; // declaring the led to be pin 6
const int led_7 = 5; // declaring the led to be pin 7

void setup() {
  pinMode(led_3, OUTPUT); // Initialize the led_3 as an output
  pinMode(led_4, OUTPUT); // Initializing the led_4 as an output
  pinMode(led_5, OUTPUT); // Initializing the led_5 as an output
  pinMode(led_6, OUTPUT); // Initializing the led_6 as an output
  pinMode(led_7, OUTPUT); // Initializng the led_7 as an output
  
  pinMode(tempPin, INPUT); // Initializing the A0 as the pin
...
{} 
void leds(){
   Serial.print("In interrupt temperatur: "); // Printing the timer interrupt message to the serial monitor
  
  celsuis = map(((analogRead(tempPin) - 20) * 3.043), 0, 1023, 0, 50); // calculates a temperature reading in celsius based on an analog voltage input from a temperature sensor
  Serial.println(celsuis); // Printing the temperatur to the serial monitor
  delay(1000); // the delay of one second 
  
  if(celsuis<=0){//if temperature lower than 0
    digitalWrite(led_3, LOW);//LED 1 off
    digitalWrite(led_4, LOW);//LED 2 off
    digitalWrite(led_5, LOW);//LED 3 off
    digitalWrite(led_6, LOW);//LED 4 off
    digitalWrite(led_7, LOW);//LED 5 off
}
if(celsuis>0&&celsuis<10){//if temperature lower than 10 but higher than 1
    digitalWrite(led_3, HIGH);//LED 1 on
    digitalWrite(led_4, LOW);//LED 2 off
    digitalWrite(led_5, LOW);//LED 3 off
    digitalWrite(led_6, LOW);//LED 4 off
    digitalWrite(led_7, LOW);//LED 5 off
...
if(celsuis>11&&celsuis<20){//if temperature lower than 20 but higher than 11
    digitalWrite(led_3, HIGH);//LED 1 on
    digitalWrite(led_4, HIGH);//LED 2 on
    digitalWrite(led_5, LOW);//LED 3 off
    digitalWrite(led_6, LOW);//LED 4 off
    digitalWrite(led_7, LOW);//LED 5 off
}
if(celsuis>21&&celsuis<30){//if temperature lower than 30 but higher than 21
    digitalWrite(led_3, HIGH);//LED 1 on
    digitalWrite(led_4, HIGH);//LED 2 on
    digitalWrite(led_5, HIGH);//LED 3 on
    digitalWrite(led_6, LOW);//LED 4 off
    digitalWrite(led_7, LOW);//LED 5 off
}
if(celsuis>31&&celsuis<40){//if temperature lower than 40 but higher than 31
    digitalWrite(led_3, HIGH);//LED 1 on
    digitalWrite(led_4, HIGH);//LED 2 on
    digitalWrite(led_5, HIGH);//LED 3 on
    digitalWrite(led_6, HIGH);//LED 4 on
    digitalWrite(led_7, LOW);//LED 5 off
}
if(celsuis>41&&celsuis<50){//if temperature lower than 50 but higher than 41
    digitalWrite(led_3, HIGH);//LED 1 on
    digitalWrite(led_4, HIGH);//LED 2 on
    digitalWrite(led_5, HIGH);//LED 3 on
    digitalWrite(led_6, HIGH);//LED 4 on
    digitalWrite(led_7, HIGH);//LED 5 on
}
}